...
function printHelp(): void {
  ...
  const lines = [
    ...
    c("doctor", "Diagnose handoff layer health (--smoke for full E2E check)"),
    ...
    c("sync", "Manual flush+pull sync (on-demand)"),
    "",
    `  ${bold("Capture")}`,
    ...
    c("move", "Reassign a conversation to a different project"),
    "",
    `  ${bold("Account")}`,
    c("reset", "Wipe all data, keep account"),
    c("uninstall", "Remove all Synapse configs"),
    c("purge-empty", "Delete empty projects (--yes to execute)"),
    "",
