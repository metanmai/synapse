...
export function startHandoffLoop(a: HandoffLoopArgs): () => void {
  ...
  async function cycle(): Promise<boolean> {
    ...
    // Auto-sync is intentionally available on every tier. A historical
    // Phase 03-05 gate made Free continuity depend on graceful hooks or a
    // manual `synapsesync sync`, which violated the product's core promise
    // when a session ended via ctrl+C, crash, or power loss. The gate was
    // removed in 3776c154; no billing lookup belongs in this loop.
