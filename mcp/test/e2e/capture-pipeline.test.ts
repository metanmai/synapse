...
describe("Capture Pipeline E2E", () => {
  ...
  // Cloud Sync (live-backend tests) removed 2026-06-10:
  //   - Used a stale /api/conversations response shape (treated the {conversations:[...]}
  //     object as a top-level array). Two of three were also returning sync()=false
  //     against the live backend with the synthetic projectPath "/tmp/cloud-sync-test"
  //     that the tests pass — needs a from-scratch rewrite, not a patch.
  //   - CloudSyncer's end-to-end behavior already has continuous coverage via
  //     scripts/e2e-cli.mjs (the sync + invite stages), scripts/e2e-adapter-roundtrip.mjs
  //     (the 6-tool capture → sync → list pipeline), and scripts/e2e-insight-roundtrip.mjs
  //     (capture → project resolve → save_insight → brief recompose) — all in the
  //     merge gate. The unit-level CloudSyncer logic is covered by scripts that
  //     mock fetch (mcp/test/unit/...). Deleting these tests doesn't drop coverage.
});
