...
autonomous: true
requirements: [BUG-02, BUG-03, BUG-04, BUGS-MD-12]

must_haves:
  truths:
    - "Every Wave 2/3 task has a failing test that pins down the expected behavior before production code is touched."
    - "No production source files in Wave 2/3 need to be created from scratch — the stubs already exist with the right exports."
...
<objective>
Wave 0 (Nyquist) scaffolding for **slice 1a-prime** (Plan 05 deferred to slice 1b on the CF-enabled machine, so OBS-01 scaffolding is also deferred). Create 2 new test files (`mcp-command.test.ts`, `daemon-backoff.test.ts`) with at least one RED (failing) test each, extend 3 existing test files (`init.test.ts`, `status.test.ts`, `os-service.test.ts`) with placeholders for the new BUG-02 / BUG-04 branches and the LAUNCHD_LABEL invariant, stub the 3 new mcp-workspace production files (`mcp-command.ts`, `daemon-supervisor.ts`, `daemon-backoff.ts`) with their exported signatures so Wave 2/3 plans can import without TypeScript errors, AND lift the launchd label out of `mcp/src/capture/os-service.ts` as a named export `LAUNCHD_LABEL` so Plan 01-02's `daemon-supervisor.ts` can import a single source of truth.

Purpose: Wave 2/3 (3 parallel implementation plans + 1 sequential Wave 3) cannot start until every failing test exists, per Nyquist validation contract. Stubs also let Wave 2/3 tasks edit a single file each without circular import / type-resolution churn. The `LAUNCHD_LABEL` export and the `computeNextDelay` extraction together eliminate two checker-flagged risks: shadowed-string drift and fake-timer pollution from preserved `setInterval` calls.

**Why `backend/src/lib/observability.ts` and the two observability test files are NOT in this plan:** Plan 05 (Sentry observability) needs `npm install @sentry/cloudflare @sentry/hono` against the npm registry, which Netskope blocks on this device. Per the pre-execution risk audit, Plan 05 is deferred to slice 1b on the CF-enabled machine. Landing observability RED tests here without their GREEN implementation would block every commit via the pre-push hook (`lint && typecheck && test`). The observability scaffolding is therefore moved to Plan 05's own Wave 0 step on the CF machine.

Output: 5 test files (2 new + 3 extended via `init.test.ts`, `status.test.ts`, `os-service.test.ts`), 3 stub source files, and 1 surgical edit to `os-service.ts` (extract launchd label as named export, template-string the plist), all committed in a single batch. The pre-push hook runs once (per CONTEXT.md "pre-push hook fires ~25s") — `test` will report N failing tests; `lint && typecheck` must pass. We intentionally let the test step fail until Wave 2 lands.

This plan is **scaffolding-only** — it has no user-observable outcome on its own. The Wave 2/3 plans (02, 03, 04) deliver the user-observable outcomes for SC#2 (daemon status surfacing) and SC#3 (wizard `.mcp.json`). SC#4 (Sentry deliberate-throw end-to-end) is the responsibility of slice 1b on the CF-enabled machine.
</objective>
...
Stub export shapes (Wave 2/3 plans depend on these):
...
- `mcp/src/capture/daemon-backoff.ts` MUST export:
    ...
    - `export function computeNextDelay(prevDelayMs: number, lastSucceeded: boolean): number` — pure function, no side effects, no timers. Plan 01-02 fills the body and wires it into `startHandoffLoop`.
...
<task type="auto" tdd="true">
  <name>Task 1: Stub the 3 new mcp-workspace production files with typed exports</name>
  <files>mcp/src/cli/util/mcp-command.ts, mcp/src/cli/util/daemon-supervisor.ts, mcp/src/capture/daemon-backoff.ts</files>
  <read_first>
    - .planning/phases/01-stabilize-backend-observability/01-CONTEXT.md (full)
    - .planning/phases/01-stabilize-backend-observability/01-RESEARCH.md §"Pattern 3" (lines 317-390 — daemon-supervisor shape), §"Pattern 4" (lines 392-480 — mcp-command shape), §"Pattern 5" (lines 482-566 — backoff math)
  ...
  <done>All 3 files exist; `npm run typecheck` exits 0 from repo root; `npm run lint` exits 0; runtime call of any stub function throws "not implemented — Wave 2".</done>
</task>

<task type="auto" tdd="true">
  <name>Task 2: Create 2 new RED test files + extend 3 existing test files (init, status, os-service)</name>
  <files>mcp/test/cli/mcp-command.test.ts, mcp/test/capture/daemon-backoff.test.ts, mcp/test/cli/init.test.ts, mcp/test/cli/status.test.ts, mcp/test/capture/os-service.test.ts</files>
  <read_first>
    - .planning/phases/01-stabilize-backend-observability/01-VALIDATION.md §"Per-Task Verification Map" (every BUG-02/03/04 + BUGS-MD-12 row — OBS-01 rows are scoped to slice 1b)
  ...
  <behavior>
    For each VALIDATION.md row (BUG-02/03/04 + BUGS-MD-12), write a test that NAMES the expected behavior in its `it()` description and CALLS the (currently-stubbed) function. These tests MUST FAIL on Wave 1 (because the stubs throw "not implemented") and PASS as Wave 2/3 lands each implementation.
  ...
  <action>
    ...
    For the `os-service.test.ts` extension, append the `describe("LAUNCHD_LABEL invariant")` block with both `it(...)` cases. These tests RUN ONLY after Task 3 lands (Wave 0 batch) — they pass immediately within this plan as soon as the LAUNCHD_LABEL export exists, so they are GREEN at the end of Plan 01-01 (unlike the BUG-02/03/04 tests which stay RED until Wave 2/3).
  </action>
  <verify>
  ...
  <acceptance_criteria>
    ...
    - All 5 affected test files exist on disk and are syntactically valid: `cd mcp && npx vitest run test/cli/mcp-command.test.ts test/capture/daemon-backoff.test.ts test/cli/init.test.ts test/cli/status.test.ts test/capture/os-service.test.ts 2>&1 | grep -q "Tests"` exits 0 (vitest reports test counts rather than refusing to load).
    - `npm run lint` exits 0 from repo root; `npm run typecheck` exits 0 from repo root.
    - No `.skip` / `it.skip` / `describe.skip` strings appear in the new test files: `grep -rE "\.skip\b" mcp/test/cli/mcp-command.test.ts mcp/test/capture/daemon-backoff.test.ts | wc -l` returns 0.
  </acceptance_criteria>
  <done>All 5 affected files exist; `npx vitest run` shows the expected failures (BUG-02: 5 failing now including the LAUNCHD_LABEL-via-sentinel test, BUG-03: 4 failing, BUG-04: 4 failing, BUGS.md #12: 5 failing = 18 RED tests total per VALIDATION.md map) plus the 2 os-service LAUNCHD_LABEL-invariant tests that turn GREEN immediately after Task 3 lands; `npm run lint && npm run typecheck` exit 0 from repo root.</done>
</task>
...
<task type="auto" tdd="false">
  ...
  <behavior>
    ...
    - Name the constant `LAUNCHD_LABEL` (not `LABEL`) — more specific name avoids accidental shadowing when callers import it.
...
|----------|-------------|
| (none in this plan) | Wave 0 (slice 1a-prime) is test scaffolding + stubs + a label-extraction refactor. No new code paths, no inputs, no network surface. Trust boundaries are introduced by Wave 2/3 plans (BUG-03 introduces a 2-second fetch to registry.npmjs.org). OBS-01 trust boundaries are owned by Plan 05 on the CF-enabled machine. |

## STRIDE Threat Register

| Threat ID | Category | Component | Disposition | Mitigation Plan |
|-----------|----------|-----------|-------------|-----------------|
| T-01-SC | Tampering | npm install (none in this plan) | n/a | This plan installs zero packages. Sentry installs occur in Plan 05 on the CF-enabled machine. |
...
4. Failures all reference the "not implemented — Wave 2" stub throws — confirming the RED step.
5. The LAUNCHD_LABEL runtime invariant (`cd mcp && npx vitest run test/capture/os-service.test.ts -t "exports LAUNCHD_LABEL"`) exits 0 — proves the named export is importable.

Pre-push hook will run on commit; expected outcome: lint pass, typecheck pass, test FAIL (intentional). Push with the hook running once at the wave boundary per CONTEXT.md guidance — commit locally, push to `tanmain/synapse` after Wave 2/3 lands. (Alternatively: commit + `git push --no-verify` for the wave-0 commit only, then push normally after Wave 2/3. Operator's call.)
</verification>

<success_criteria>
- All 9 file paths in `files_modified` exist on disk with the contents described.
- VALIDATION.md "Wave 0 Requirements" checklist can be ticked for slice 1a-prime: 2 new test files exist, 3 existing test files extended (`init.test.ts`, `status.test.ts`, `os-service.test.ts`). The 2 observability test files in the original VALIDATION.md list are deferred to Plan 05's Wave 0 on the CF-enabled machine.
- Wave 2/3 plans (02, 03, 04) can each touch their owned files without needing to also create supporting test or stub files.
- 18 RED tests are queued for Wave 2/3, mapped to the VALIDATION.md "Per-Task Verification Map" rows that Wave 2/3 will turn green. Plus 2 invariant tests in `os-service.test.ts` (LAUNCHD_LABEL runtime + render-equivalence) that turn GREEN immediately after Task 3 lands within this plan.
...
- **Scaffolding-only — no user-observable outcome on its own; Wave 2/3 plans satisfy SC#2 (BUG-02 status surfacing) and SC#3 (BUG-03 `.mcp.json` shape).** Slice 1b satisfies SC#4 (Sentry deliberate-throw end-to-end) on the CF-enabled machine.
</success_criteria>

<output>
Create `.planning/phases/01-stabilize-backend-observability/01-01-SUMMARY.md` when done. Note in the summary which VALIDATION.md rows are queued RED for Wave 2/3, confirm the LAUNCHD_LABEL runtime invariant + render-equivalence tests pass at the end of this plan, and confirm that `LAUNCHD_LABEL` is now exported from `os-service.ts`.
</output>
