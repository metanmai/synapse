...
<!-- Stabilize-for-launch milestone — target end of this week (2026-05-22 / 2026-05-23). -->

- [ ] **REQ-BUG-01** — Backend `/api/events/batch` Cloudflare 1101 fixed; daemon flushes succeed end-to-end (BUGS.md #1, P0)
- [ ] **REQ-BUG-02** — `synapse capture status` accurately reports daemon state when supervised by launchd / systemd (BUGS.md #2, P1)
- [ ] **REQ-BUG-03** — Wizard's MCP configs work on proxy-restricted networks (`npx synapsesync` fallback, BUGS.md #3, P1)
- [ ] **REQ-BUG-04** — `synapse init` writes the project-local `.mcp.json` so MCP server is reachable in cwd (BUGS.md #4, P1)
- [ ] **REQ-MEASURE-01** — Brief-usefulness signal: thumbs Y/N rating on each SessionStart brief, stored server-side, viewable on dashboard
- [ ] **REQ-MEASURE-02** — Time-to-context signal: daemon-emitted timestamp at session start + at "first non-orientation message"; persisted server-side
- [ ] **REQ-MEASURE-03** — Minimal observability view (per-project trend of rating rate + median time-to-context) on dashboard
- [ ] **REQ-LAUNCH-01** — synapsesync.app public signup flow accepts new users behind a waitlist throttle (signups queued, granted access in controlled batches)
- [ ] **REQ-LAUNCH-02** — Email notification when a waitlist signup is granted access (the user can sign in and run the wizard)
- [ ] **REQ-LAUNCH-03** — Landing-page-to-installed-daemon path verified for a fresh-laptop scenario (manual rehearsal, not just automated)
- [ ] **REQ-DOGFOOD-01** — Solo dogfood: this developer's daily Claude Code sessions run captured + briefed for ≥3 consecutive days pre-launch with no manual intervention
...
- **Team / multi-user collaboration features** beyond what backend already supports — invites flow exists but no new sharing UX
...
- **Timeline**: Launch by **end of this week** (2026-05-22 / 2026-05-23) — ~5 days from today. Forces ruthless prioritization; only stabilization + telemetry + waitlist-throttle scope fits
...
| Per-device CLI keys with `cli-<sanitized-hostname>` labels | Earlier model silently invalidated old devices on each sign-in; new model adds devices independently with tier limits + revoke-and-resign for the limit case | ✓ Good (shipped today, verified end-to-end) |
