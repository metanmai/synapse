...
- **Supabase auto-migrate — verified 2026-07-18.** Run `29649638136` attempt 2 reached `supabase db push`, removed the stale remote-only `000 delete_user` ledger row, applied migrations 031 and `20260717170215`, and passed.
- **Migration/RLS recurrence guard — complete 2026-07-18.** `npm run lint:migrations` scans the ordered migration chain, fails when a surviving public table ends with RLS disabled, and runs in local `verify` plus both CI operating-system jobs. All 24 existing public tables pass; parser regression tests cover later enablement, disablement, drops, quoted identifiers, and SQL noise.
...
- **SessionStore `(tool, session_id)` keying — fixed 2026-07-18.** New files are stored under a versioned source/session hierarchy, reads and deletes require both key parts, and legacy flat files remain readable and migrate on the next save. Same-ID sessions from different tools now coexist independently.
...
- **Background recompute upload retry — fixed 2026-07-18.** Precomputed `POST /compact` uploads now make three bounded attempts for network errors, 408, 429, and 5xx responses. Local compaction runs exactly once; permanent 4xx responses fail fast; exhausted retries degrade to the cached handoff without invoking a second LLM path.
...
- 2026-07-17/18: **Production Supabase security and automation close-out.** Production drift was repaired with `20260717170215_harden_public_schema_rls.sql` (`454af70e`): RLS and browser-role grants were hardened for `project_context` and `deleted_accounts`; six analytics views became security-invoker; seven privileged functions had fixed search paths and restricted execution; Metabase read-only and service-role access were preserved. Migration 031 (`api_keys.scope`) was applied and verified. Supabase advisor errors fell to zero. After `SUPABASE_DB_PASSWORD` was added to GitHub `prod`, run `29649638136` attempt 2 reached `supabase db push`, reconciled the stale remote-only `000 delete_user` migration ledger entry, applied the two outstanding migrations, and passed. Migration `20260718154701_move_vector_extension.sql` then moved pgvector to `extensions`, restored the missing entry-semantic-search schema, and cleared the extension warning without breaking the three backend vector RPCs. Leaked-password protection is the only remaining advisor warning and requires upgrading from Supabase Free. The recurrence-level migration/RLS lint now guards all 24 public tables in local verification and CI.
