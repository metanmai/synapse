...
import {
	...
	upsertSubscription,
} from "../../src/db/queries/subscriptions";
...
import {
	...
	getShareLinkByToken,
	deleteShareLink,
} from "../../src/db/queries/share-links";
...
function createSequentialMockDb(
	...responses: { data?: unknown; error?: unknown; count?: number | null }[]
) {
	let callIndex = 0;
	...
	const from = vi.fn().mockImplementation(() => {
		...
		(chainable as any).then = (resolve: (v: unknown) => void) => resolve(resp);
...
describe("entries queries", () => {
	...
	describe("upsertEntry — new entry", () => {
		it("queries entries, then inserts when no existing row", async () => {
			const created = {
				id: "e1", project_id: "p1", path: "notes/a.md",
				content: "hello", content_type: "markdown", author_id: null,
				source: "claude", tags: [], google_doc_id: null,
	...
	describe("upsertEntry — existing entry", () => {
		it("saves to history then updates when entry exists", async () => {
			const existing = {
				id: "e1", project_id: "p1", path: "notes/a.md",
				content: "old", content_type: "markdown", author_id: null,
				source: "human", tags: ["x"], google_doc_id: null,
				created_at: "2026-01-01", updated_at: "2026-01-01",
			};
			...
			expect(db.chains[2].update).toHaveBeenCalledWith(
	...
	describe("getEntry", () => {
		it("queries entries with project_id and path filters", async () => {
			const row = {
				id: "e1", project_id: "p1", path: "docs/a.md",
	...
	describe("deleteEntry", () => {
		...
		it("throws when Supabase returns an error", async () => {
			...
			await expect(
				deleteEntry(db as unknown as SupabaseClient, "p1", "x"),
...
describe("projects queries", () => {
	...
	describe("createProject", () => {
		...
		it("throws if project insert fails", async () => {
			...
			await expect(
				createProject(db as unknown as SupabaseClient, "dup", "u1"),
	...
	describe("getProjectByName", () => {
		...
		it("falls back to shared membership when not owned", async () => {
			...
			expect(db.chains[1].select).toHaveBeenCalledWith(
...
describe("api-keys queries", () => {
	...
	describe("findUserByApiKeyHash", () => {
		...
		it("throws ApiKeyExpiredError when key is expired", async () => {
			...
			await expect(
				findUserByApiKeyHash(db as unknown as SupabaseClient, "hash123"),
	...
	describe("createApiKey", () => {
		it("inserts with correct fields", async () => {
			const key = {
				id: "ak1", user_id: "u1", key_hash: "h", label: "My Key",
				expires_at: "2099-01-01", last_used_at: null, created_at: "2026-01-01",
			};
			const db = mockSuccess(key);

			const result = await createApiKey(
				db as unknown as SupabaseClient, "u1", "h", "My Key", "2099-01-01",
			);
...
describe("subscriptions queries", () => {
	...
	describe("getActiveSubscription", () => {
		it("filters by status IN [active, past_due]", async () => {
			const sub = {
				id: "s1", user_id: "u1", provider: "creem",
...
describe("share-links queries", () => {
	...
	describe("createShareLink", () => {
		it("inserts with correct fields", async () => {
			const link = {
				id: "sl1", project_id: "proj1", token: "tok123",
				role: "viewer", created_by: "u1", expires_at: "2099-01-01",
			};
			const db = mockSuccess(link);

			const result = await createShareLink(
				db as unknown as SupabaseClient, "proj1", "viewer", "u1", "2099-01-01",
			);
	...
	describe("deleteShareLink", () => {
		...
		it("throws when deletion fails", async () => {
			...
			await expect(
				deleteShareLink(db as unknown as SupabaseClient, "proj1", "tok123"),
			).rejects.toEqual(expect.objectContaining({ message: "permission denied" }));
		});
	});
});
